...
when related to matrices. I don't know who first game up with these names.
...
*   camera matrix
    ...
    a matrix that positions the camera in the world
...
*   projection matrix
    ...
    a matrix and converts a frustum of space into clip space

*   local matrix

    when using a scene graph the matrix at any particular node on the graph
    before mutlipying with any other nodes.
