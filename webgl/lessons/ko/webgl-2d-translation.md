...
이동(Translation)는 무언가를 "움직인다"라는 의미를 가진 멋진 수학적 이름입니다. 물론 여기서는 기하학을 말하는게 적합하지만 문장을 영어에서 일본어로 옮기는 것을 생각해 봅시다. [첫번째 게시글](webgl-fundamentals.html)에 있는 예제 코드의 `setRectangle`에 변경된 값을 전달하여 사각형을 쉽게 이동할 수 있습니다. 여기에 [이전 예제](webgl-fundamentals.html)를 기반으로 한 예제가 있습니다.
